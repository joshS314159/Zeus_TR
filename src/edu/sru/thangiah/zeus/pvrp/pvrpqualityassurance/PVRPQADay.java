...
//CONSTRUCTOR
public boolean checkDistanceConstraint(PVRPQADay day) {
	//VARIABLES
	boolean isDiagnostic = true;        //print diagnostic information?
	boolean status = true;                //return value
	double totalDistance = 0;            //
	double distance = 0;                //
	PVRPQANode nodeOne;                    //these are for
	PVRPQANode nodeTwo;                    //distance calculations
	int distanceOne;                    //
	int distanceTwo;                    //
	...
	nodeOne = (PVRPQANode) getNodes().elementAt(0);


	//COMPUTE TOTAL TRAVEL DISTANCE IN THE ROUTE
	for(int i = 1; i < getNodes().size(); i++) {
		nodeTwo = (PVRPQANode) getNodes().elementAt(i);
...
public boolean checkCapacityConstraint(PVRPQADay day) {
	//VARIABLES
