...
public class PVRPDaysLLCostFunctions
...
public double getTotalCost(Object o) {
	...
	return ((PVRPDayLinkedList) o).getAttributes().getTotalCost();
}
...
public float getTotalDemand(Object o) {
	...
	return (int) ((PVRPDayLinkedList) o).getAttributes().getTotalDemand();
}
...
public double getTotalDistance(Object o) {
	...
	return ((PVRPDayLinkedList) o).getAttributes().getTotalDistance();
}
...
public void setTotalCost(Object o) {
	PVRPDayLinkedList daysList = (PVRPDayLinkedList) o;
	PVRPDay theDay = ((PVRPDayLinkedList) o).getHead();
	double cost = 0;

	while(theDay.getNext() != daysList.getTail()) {
		theDay = theDay.getNext();
		cost += PVRPProblemInfo.daysLevelCostF.getTotalCost(theDay);
	}
...
public void setTotalDemand(Object o) {
	...
	while(theDay.getNext() != daysList.getTail()) {
		theDay = theDay.getNext();
		if(!theDay.isEmptyMainNodes()) {
			daysList.getAttributes().setTotalDemand(
					daysList.getAttributes().getTotalDemand() + PVRPProblemInfo.daysLevelCostF.getTotalDemand(theDay));
		}
	}
}
	...
	while(theDay.getNext() != daysList.getTail()) {
		...
		if(!theDay.isEmptyMainNodes()) {
			daysList.getAttributes().setTotalDistance(daysList.getAttributes().getTotalDistance() +
													  PVRPProblemInfo.daysLevelCostF.getTotalDistance(theDay));
		}
	}
