...
TRShipmentsList   mainShipments;
TRQADepotsList    pvrpQADepots;
TRQAShipmentsList pvrpQAShipments;
File              shipmentFile;
...
//CONSTRUCTOR
public TRQA(TRDepotsList mainDepots, TRShipmentsList mainShipments)
		throws IOException {
	this.mainDepots = mainDepots;
	this.mainShipments = mainShipments;
	pvrpQAShipments = new TRQAShipmentsList();
	pvrpQADepots = new TRQADepotsList();
...
public void readShipmentFile()
	...
	TRQAShipment theShipment = null;                    //a pvrpQAshipment
...
public void readSolutionFile()
	...
	TRQADepot theDepot;            //HOLDS A DEPOT QA
	TRQATruck theTruck;            //HOLDS A TRUCK QA
	...
	for(int depotCounter = 0; depotCounter < numberOfDepots; depotCounter++) {
		...
		for(int truckCounter = 0; truckCounter < numberOfTrucks; truckCounter++) {
			...
			theTruck = new TRQATruck();            //MAKE NEW TRUCK
			...
			for(int truckColumn = 0; truckColumn < NUMBER_COLUMNS; truckColumn++) {
				...
				switch(truckColumn) {
					...
					case 2:    //READ TRUCK TYPE
						...
						break; //IS ALWAYS THIS VALUE FOR TR
					case 4:    //READ DEMAND
